...
function t_symmetry()
  p = base_params();
  [b, ~] = design_fir_pm(p);
  assert_near(b, fliplr(b), 1e-10, 'remez output must be symmetric (linear phase)');
end

function t_lowpass_dc_gain()
  p = base_params();
  [b, ~] = design_fir_pm(p);
  % Equiripple passband means DC gain deviates by the ripple amount delta;
  % for a well-specified design this is well under 0.05.
  assert_near(sum(b), 1.0, 0.05, 'lowpass remez DC gain must be ~1');
end
...
function t_bandpass_length()
  ...
  assert_near(b, fliplr(b), 1e-10, 'bandpass remez must be symmetric');
end
...
function t_bandstop_length()
  ...
  assert_near(b, fliplr(b), 1e-10, 'bandstop remez must be symmetric');
end
...
function t_three_band()
  ...
  assert_near(b, fliplr(b), 1e-10, 'three-band remez must be symmetric');
end
