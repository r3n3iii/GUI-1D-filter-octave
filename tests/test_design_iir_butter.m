%% DESCRIPTION
%  Unit tests for design_iir_butter.
%% INPUTS
%  none
%% OUTPUTS
%  none — throws on failure

function test_design_iir_butter()
  t_coeff_length();
  t_stability();
  t_lowpass_passband_gain();
  t_lowpass_stopband_gain();
  t_highpass_passband_gain();
  t_highpass_stopband_gain();
  t_bandpass_gain();
  t_bandstop_gain();
end

function t_coeff_length()
  p = base_params();
  [b, a] = design_iir_butter(p);
  if numel(b) ~= p.order + 1
    error('t_coeff_length: b should have order+1 elements, got %d', numel(b));
  end
  if numel(a) ~= p.order + 1
    error('t_coeff_length: a should have order+1 elements, got %d', numel(a));
  end
end

function t_stability()
  p = base_params();
  [~, a] = design_iir_butter(p);
  poles = roots(a);
  if any(abs(poles) >= 1)
    error('t_stability: Butterworth filter has poles outside the unit circle');
  end
end

function t_lowpass_passband_gain()
  p = base_params();
  [b, a] = design_iir_butter(p);
  if eval_gain(b, a, 0.1) < 0.9
    error('t_lowpass_passband_gain: gain at f=0.1 should be > 0.9');
  end
end

function t_lowpass_stopband_gain()
  p = base_params();
  [b, a] = design_iir_butter(p);
  if eval_gain(b, a, 0.5) > 0.1
    error('t_lowpass_stopband_gain: gain at f=0.5 should be < 0.1');
  end
end

function t_highpass_passband_gain()
  p = base_params();
  p.band = 'high';
  [b, a] = design_iir_butter(p);
  if eval_gain(b, a, 0.45) < 0.9
    error('t_highpass_passband_gain: gain at f=0.45 should be > 0.9');
  end
end

function t_highpass_stopband_gain()
  p = base_params();
  p.band = 'high';
  [b, a] = design_iir_butter(p);
  if eval_gain(b, a, 0.1) > 0.1
    error('t_highpass_stopband_gain: gain at f=0.1 should be < 0.1');
  end
end

function t_bandpass_gain()
  p.order = 4;
  p.Wn    = [0.2 0.5];
  p.band  = 'bandpass';
  [b, a] = design_iir_butter(p);
  if eval_gain(b, a, 0.35) < 0.9
    error('t_bandpass_gain: gain at f=0.35 (mid-passband) should be > 0.9');
  end
  if eval_gain(b, a, 0.05) > 0.1
    error('t_bandpass_gain: gain at f=0.05 (stopband) should be < 0.1');
  end
  poles = roots(a);
  if any(abs(poles) >= 1)
    error('t_bandpass_gain: bandpass filter is unstable');
  end
end

function t_bandstop_gain()
  p.order = 4;
  p.Wn    = [0.2 0.5];
  p.band  = 'stop';
  [b, a] = design_iir_butter(p);
  if eval_gain(b, a, 0.05) < 0.9
    error('t_bandstop_gain: gain at f=0.05 (passband) should be > 0.9');
  end
  if eval_gain(b, a, 0.35) > 0.1
    error('t_bandstop_gain: gain at f=0.35 (stopband) should be < 0.1');
  end
end

function gain = eval_gain(b, a, f_norm)
  z = exp(1j * pi * f_norm);
  gain = abs(polyval(b, z) / polyval(a, z));
end

function p = base_params()
  p.order = 4;
  p.Wn    = 0.3;
  p.band  = 'low';
end
