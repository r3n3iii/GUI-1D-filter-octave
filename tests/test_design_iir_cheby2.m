%% DESCRIPTION
%  Unit tests for design_iir_cheby2.
%% INPUTS
%  none
%% OUTPUTS
%  none — throws on failure

function test_design_iir_cheby2()
  t_coeff_length();
  t_stability();
  t_lowpass_passband_gain();
  t_lowpass_stopband_gain();
  t_stopband_edge_gain();
  t_highpass_gains();
  t_bandpass_gains();
end
...
function t_stability()
  p = base_params();
  [~, a] = design_iir_cheby2(p);
  poles = roots(a);
  if any(abs(poles) >= 1)
    error('t_stability: Chebyshev II filter has poles outside the unit circle');
  end
end

function t_lowpass_passband_gain()
  % Cheby2 passband is monotonic — gain well below Wn should be close to 1.
  p = base_params();
  [b, a] = design_iir_cheby2(p);
  if eval_gain(b, a, 0.1) < 0.9
...
function t_stopband_edge_gain()
  % At f=Wn, Chebyshev II gain equals exactly 10^(-Rs/20).
  p = base_params();
  [b, a] = design_iir_cheby2(p);
  expected = 10^(-p.Rs / 20);
  assert_near(eval_gain(b, a, p.Wn), expected, 0.005, 'cheby2 stopband edge gain');
end
...
function t_highpass_gains()
  ...
  p.band = 'high';
  [b, a] = design_iir_cheby2(p);
  % Cheby2 Wn=0.3 is the stopband edge; the passband begins above that
  % with a transition zone — check well into the passband at f=0.7.
  if eval_gain(b, a, 0.7) < 0.9
    error('t_highpass_gains: passband gain at f=0.7 should be > 0.9');
  end
...
function t_bandpass_gains()
  ...
  p.band  = 'bandpass';
  [b, a] = design_iir_cheby2(p);
  if eval_gain(b, a, 0.35) < 0.9
    error('t_bandpass_gains: mid-passband gain at f=0.35 should be > 0.9');
  end
